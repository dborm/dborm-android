...
public class SelectJoinTest extends BaseTest {
    ...
    public void testUseBaseDomain() {
        ...
        assertEquals("true", baseDomain.getParam("looked"));//注意:集合中存放的数据的类型全部是String类型的
        assertEquals("15", baseDomain.getParam("readTime"));//注意:集合中存放的数据的类型全部是String类型的
    }
    ...
    public void testSelectTeam() {
        ...
        for (int i = 0; i < entityList.size(); i++) {
             ...
             */
            assertEquals(BOOK_ID, bookInfo.getId());
            boolean result = bookInfo.getLooked();
